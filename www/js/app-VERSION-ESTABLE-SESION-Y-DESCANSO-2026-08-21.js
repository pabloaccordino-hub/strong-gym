...
function restoreActiveWorkout(){
    ...
    try{
        ...
        /*
         * Esperar a que la interfaz termine de cargar.
         */
        ...
        setTimeout(
            () => {
                ...
                try{
                    ...
                    console.log(
                        "STRONG GYM: entrenamiento activo restaurado."
                    );

                }catch(error){

                    console.error(
                        "STRONG GYM: error restaurando entrenamiento:",
                        error
                    );
...
/*
 * Hora exacta en la que termina el descanso.
 * El contador real no dependerá de setInterval().
 */
let strongGymRestTimerEndTime = null;
...
function startStrongGymRestTimer(
    ...
    /*
     * Si existe un descanso guardado, recuperar su
     * hora REAL de finalización.
     */
    if(
        restoredRestState &&
        restoredRestState.endTime
    ){
        ...
        strongGymRestTimerTotal =
            Number(
                restoredRestState.total
            ) ||
            Number(
                restoredRestState.seconds
            ) ||
            90;
...
function startWorkoutMode(
    routine,
    selectedDay = null,
    restoredWorkoutState = null
){

    const workoutStartTime =
        restoredWorkoutState &&
        restoredWorkoutState.workoutStartTime
            ? restoredWorkoutState.workoutStartTime
            : Date.now();
     ...
     * =====================================================
     *
     * Recuperamos los valores guardados de cada serie.
     */
    const restoredSetValues =
        restoredWorkoutState &&
        Array.isArray(
            restoredWorkoutState.setValues
        )
            ? restoredWorkoutState.setValues
            : [];
...
return `
    ...
    /*
     * =========================================================
     * STRONG_GYM_RESTORE_SET_VALUES
     * =========================================================
     *
     * Aplicar inmediatamente los valores de la sesión
     * interrumpida al HTML recién creado.
     */

    restoredSetValues.forEach(
        savedSet => {

            const row =
                modal.querySelector(
                    `.workout-set[data-exercise-index="${savedSet.exerciseIndex}"][data-set-index="${savedSet.setIndex}"]`
                );
    ...
    /*
     * =========================================================
     * GUARDAR SESIÓN DE ENTRENAMIENTO ACTIVA
     * =========================================================
     */
    ...
    function saveActiveWorkoutState(){
        ...
        try{
            ...
            modal.querySelectorAll(
                ...
                setRow => {
                    ...
                    completedSets.push({
                        ...
                        exerciseIndex:
                            Number(
                                setRow.dataset.exerciseIndex
                            ),

                        setIndex:
                            Number(
                                setRow.dataset.setIndex
                            ),
            ...
            /*
             * Guardamos también todos los valores,
             * aunque la serie todavía no esté completada.
             */
            ...
            const state = {
                ...
                /*
                 * Estado del descanso automático.
                 *
                 * Guardamos la hora REAL de finalización
                 * para poder reconstruir el descanso aunque
                 * STRONG GYM haya quedado en segundo plano.
                 */
                restTimer:
                    strongGymRestTimerEndTime
                        ? {
            ...
            console.log(
                "STRONG GYM: sesión activa guardada.",
                state
            );
            ...
            console.error(
                "STRONG GYM: error guardando sesión activa:",
                error
            );
    ...
    /*
     * Guardar cambios de peso, repeticiones y RIR.
     */

    modal.querySelectorAll(
        ".workout-weight, .workout-reps, .workout-rir"
    ).forEach(
        field => {
        ...
        button => {
            ...
            button.addEventListener(
                ...
                () => {
                    ...
                    /*
                     * Iniciar primero el descanso.
                     * De esta forma se genera endTime.
                     */
                    ...
                    /*
                     * Guardar DESPUES de iniciar el descanso.
                     */
    ...
    requestAnimationFrame(
        () => {
             ...
             * =================================================
             *
             * No depender solamente de endTime.
             * startStrongGymRestTimer() ya sabe calcular
             * el tiempo real restante utilizando endTime.
             */

            if(
                restoredWorkoutState &&
                restoredWorkoutState.restTimer
            ){
                ...
                const remaining =
                    Number.isFinite(
                        ...
                        : Number(
                            restoredRest.seconds
                        ) || 0;


                console.log(
                    "STRONG GYM: comprobando descanso recuperado:",
                    {
                        endTime,
                        remaining,
                        restTimer:
                ...
                if(
                    ...
                    console.log(
                        "STRONG GYM: descanso activo restaurado correctamente."
                    );
